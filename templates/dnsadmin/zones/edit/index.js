...
if (editMatch) {
  ...
  form.action = '<%== config->{manager}->{url} %>dnsadmin/' + zoneId;
  ...
  fetch('<%== config->{manager}->{url} %>/dnsadmin/' + zoneId, { headers: { 'Accept': 'application/json' } })
  ...
  // New zone mode
  form.action = '<%== config->{manager}->{url} %>/dnsadmin';
  ...
  fetch(action, {
    ...
    .then(result => {
      ...
      if (result.success) {
        ...
        setTimeout(() => { window.location.href = '<%== config->{manager}->{url} %>/dnsadmin'; }, 2000);
      }
    })
    .catch(error => {
      // Silent error handling
