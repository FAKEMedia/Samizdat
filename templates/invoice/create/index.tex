...
%   $vat =~ tr/\./,/;
% }
% if ('eur' eq $customer->{currency} && (!$customer->{billinglang} || $customer->{billinglang} !~ /^(sv)/i)) {
{\huge \textbf {\
%   if (exists($invoice->{kreditfakturaavser}) && $invoice->{kreditfakturaavser}) {
<%== __x('Credited amount incl. VAT: {totalcost} {currency}', totalcost => $invoice->{totalcost},	currency => $currency->{ $customer->{currency} }) %>\
%   } else {
<%== __x('Amount to pay incl. VAT: {totalcost} {currency}', totalcost => $invoice->{totalcost},	currency => $currency->{ $customer->{currency} }) %>\
%   }
}}
\vfill
\footnotesize {\textbf {<%== __('Net amount') %>:} <%== $invoice->{net_amount} %> <%=$currency->{ $customer->{currency} } %>}\newline
...
\footnotesize {\textbf {<%== __x('VAT {rate}\%', rate => $vat) %>:} <%== $invoice->{vatcost} %> <%=$currency->{ $customer->{currency} } %>}\newline
%   if ($invoice->{diff}) {
\footnotesize {\textbf {<%== __('Rounding') %>:} <%== $invoice->{diff} %> <%=$currency->{ $customer->{currency} } %>}\newline
%   }
% }
% if (!(exists($invoice->{kreditfakturaavser}) && $invoice->{kreditfakturaavser})) {
