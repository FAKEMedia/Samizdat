...
function deleteCookie(cname) {
    document.cookie = cname + "=logout; expires=Sat, 23 Oct 2021 13:40:42 GMT; domain=.fakenews.com; path=/; secure; SameSite=None; Max-Age=0";
}
...
function checkUsername() {
    ...
    if (userdata) {
        ...
        if ("" !== username) {
            ...
            $("#userdropdown a.pp").attr('href', '/');
            $(".username").html(username);
            $(".displayname").html(displayname);
            $("#messages").html(messages);
        }
        if (superadmin) {
            $("#memberpanel").children().addClass("superadmin");
        }
    }
}
...
$(function(){
    ...
    $('#modal').on('show.bs.modal', function (event) {
        ...
        method = $( ref ).attr('data-method');
        var modal = $(this);
        $("#modalDialog").load( url );
    });
    $(".magiclink").on("click, submit", function( e ) {
        e.preventDefault();
